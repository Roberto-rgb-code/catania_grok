...
const Especiales = () => {
  ...
  // URL base para el backend de especiales desde el archivo de entorno
  ...
  const fetchEspeciales = async () => {
    ...
    try {
      ...
      console.log('Respuesta completa en Especiales.jsx:', response.data); // Depuración
  ...
  const getPaginatedProducts = () => {
    const allProducts = filteredEspeciales.flatMap(tipo => tipo.productos);
    const startIndex = (currentPage - 1) * productsPerPage;
    const endIndex = startIndex + productsPerPage;
    return allProducts.slice(startIndex, endIndex);
  };

  const totalProducts = filteredEspeciales.reduce((acc, tipo) => acc + tipo.productos.length, 0);
        ...
        <div className="products-main">
          ...
          <div className="especiales-grid">
            {getPaginatedProducts().map((especial) => {
              const imageSrc = especial.fotos && especial.fotos.length > 0
